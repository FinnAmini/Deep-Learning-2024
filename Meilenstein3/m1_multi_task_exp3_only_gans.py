...
if __name__ == "__main__":
    ...
    train_ds, val_ds = load_dataset_from_directory(
        "../Meilenstein2/data/training_only_gans", "../Meilenstein2/data/labels_only_gans/train", batch_size=16, multi_task=True
    )
...
for freeze in [False]:
    for archname, arch in base_archs.items():
        ...
        train_log_dir = f"logs/mt_only_gans/{model_name}_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
        ...
        os.makedirs("models/mt_only_gans", exist_ok=True)
        model.save(f"models/mt_only_gans/{model_name}.keras")
